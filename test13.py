...
# prompt ChatGPT o3-mini-high
# Write me a python code to use coppeliasim to move UR3 robot arm to specific (x,y,z) input (by keybroad).
# From the code, this code is use (x, y, z) to move UR3 arm to the point. But that not is what I want. What I want is Python code to get input as (x, y, z, alpha, beta, gamma), do an inverse kinematic to rotate the arm to specific point and fix the base of the robot arm at (0, 0, 0).
...
def main():
    ...
    try:
        while True:
            ...
            print(f"End-effector target moved slowly to position x={x}, y={y}, z={z} "
                  f"with orientation alpha={alpha}, beta={beta}, gamma={gamma}")
